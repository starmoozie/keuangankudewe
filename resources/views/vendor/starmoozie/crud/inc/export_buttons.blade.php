@if ($crud->exportButtons())
  ...
  <script>
    window.crud.dataTableConfiguration.buttons = [
        {
            extend: 'collection',
            text: '<i class="la la-download"></i> {{ trans('starmoozie::crud.export.export') }}',
            dropup: true,
            buttons: [
                {
                    name: 'copyHtml5',
                ...
                },
                {
                    ...
                    action: function(e, dt, button, config) {
                        config.title = capitalize(window.location.pathname.split("/").pop());
                        config.messageTop = getDate();
                        crud.responsiveToggle(dt);
        ...
        {
            extend: 'colvis',
            text: '<i class="la la-eye-slash"></i> {{ trans('starmoozie::crud.export.column_visibility') }}',
            columns: function ( idx, data, node ) {
    ...
    function getDate() {
        ...
        let myParam   = urlParams.get('dates');

        if (!myParam) {
            return "All time";
        }

        const dates   = JSON.parse(myParam);
        const from    = new Date(dates.from).toLocaleDateString();
        const to      = new Date(dates.to).toLocaleDateString();
