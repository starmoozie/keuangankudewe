{{-- Select2 Ajax Starmoozie CRUD filter --}}

@php
    $filter->options['quiet_time'] = $filter->options['quiet_time'] ?? $filter->options['delay'] ?? 500;
	$rules = $filter->options['rules'] ?? [];
	$rules = is_array($rules) ? json_encode($rules) : $rules;
@endphp
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            $('#filter_{{ $filter->key }}').each(function () {
            	...
            	$(this).select2({
					...
					var val = arrayToString($(this).val());

					var val_text = arrayToString($(this).select2('data')[0] ? [$(this).select2('data')[0].text] : []);
		...
		const arrayToString = (item) => {
			...
			if (Array.isArray(item)) {
				...
				var values = item.filter(function(e){ return e === 0 || e });
